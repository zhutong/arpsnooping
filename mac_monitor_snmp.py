...
class SNMPHelper():
    ...
    def get_cdp_info(self):
        ...
        error_indication, error_interface_types, error_index, var_bind_table = cmd_gen.bulkCmd(
            ...
            self.transport_target,
            0, 20,
            '1.3.6.1.4.1.9.9.23.1.2.1.1.6',  # cdp neighbor device id
            '1.3.6.1.4.1.9.9.23.1.2.1.1.7',  # cdp remote interface name
        )
        ...
        if error_indication:
            ...
            if error_interface_types:
                ...
                for varBindTableRow in var_bind_table:
                    ...
                    mib_tuple = o0._value
                    if mib_tuple[:-2] != (1, 3, 6, 1, 4, 1, 9, 9, 23, 1, 2, 1, 1, 6):
                        break
                    if_index = mib_tuple[-2]
                    remote_port = remote_port._value.decode()
                    neighbor = neighbor._value.decode().split('.')[0].split('(')[0]
                    cdp_info[if_index] = dict(remote_port=remote_port,
                                              neighbor=neighbor)
...
if __name__ == '__main__':
    ...
    threads = configs['polling']['threads']
    ...
    try:
        ...
        logger.info('MAC monitoring stopped')